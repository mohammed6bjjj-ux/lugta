...
SOURCE = ROOT / "store_assets" / "source" / "app_store_ar"
TEMPLATES = ROOT / "store_assets" / "source" / "google_play_ar"
OUTPUT = ROOT / "store_assets" / "app_store" / "production_ar"
...
def build_background(kind: str) -> Image.Image:
    ...
    # The official exports contain useful RGB artwork under a mostly transparent
    # alpha channel. RGB conversion restores the intended white/brand fields.
    background = Image.open(TEMPLATES / filename).convert("RGB")
    return background.resize(CANVAS_SIZE, Image.Resampling.LANCZOS).convert("RGBA")
...
def add_header(
    ...
    *,
    ...
    logo = trimmed_logo((300, 128))
    canvas.alpha_composite(logo, (82, 70))

    title_font = font(TITLE_FONT, 76)
    body_font = font(BODY_FONT, 41)
    right = 1000
    draw.text(
        (right, 255),
        rtl(title),
    ...
    draw.rounded_rectangle((right - 134, 460, right, 475), radius=99, fill=accent)
    ...
    *,
    ...
    shadow_draw.rounded_rectangle(
        (
            phone_x - frame + 5,
            phone_y - frame + 28,
            phone_x + phone_width + frame + 5,
            phone_y + phone_height + frame + 28,
        ),
        radius=radius + frame,
        fill=(24, 19, 72, 100),
    )
...
def make_contact_sheet(paths: list[Path]) -> Path:
    ...
    sheet = Image.new(
        ...
        (
            ...
            padding + rows * (thumb_size[1] + 94 + padding),
        ),
        (241, 240, 250),
    )
    draw = ImageDraw.Draw(sheet)
    label_font = font(TITLE_FONT, 28)
    for index, path in enumerate(paths):
        ...
        y = padding + row * (thumb_size[1] + 94 + padding)
        sheet.paste(image, (x, y))
        draw.text(
            (x + thumb_size[0] // 2, y + thumb_size[1] + 42),
...
def make_zip(paths: list[Path]) -> Path:
    destination = OUTPUT / "lugta-app-store-ar-1320x2868.zip"
